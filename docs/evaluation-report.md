...
> The figures below are generated by `python -m app.evaluation`, which writes
> `evidence/metrics.json` and `evidence/evaluation_output.txt`. They are not
> hand-transcribed: re-running the command on the deterministic database
